...
if game.CoreGui:FindFirstChild('TheQuarUILib') then
    game.CoreGui:FindFirstChild('TheQuarUILib'):Destroy()
    destroyed = true
end
...
local TheQuarUILib = Instance.new("ScreenGui")

TheQuarUILib.Name = "TheQuarUILib"

protect_gui(TheQuarUILib)
...
function library:Keybind(key)
    ...
    keybindConnection = uis.InputBegan:Connect(function(input, gp)
        if not gp and input.KeyCode == Enum.KeyCode[key] then
            TheQuarUILib.Enabled = not TheQuarUILib.Enabled
        end
    end)
end
